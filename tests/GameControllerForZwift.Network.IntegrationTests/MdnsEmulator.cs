...
{
    public class MdnsEmulator
    {
        //const int Port = 36867;
        ...
        readonly MulticastService mdns = new MulticastService();
        ...
        public async Task StartAsync(CancellationToken cancellation = default)
        {
            ...
            // Start mDNS multicast service
            mdns.Start();
            mdns.QueryReceived += Mdns_QueryReceived; // optional: answer queries manually if desired

            // Advertise a service profile
            // Instance name and service type should match expected clients
            var profile = new ServiceProfile(
                "KICKR BIKE SHIFT B84D",
                "_wahoo-fitness-tnp._tcp.local",
                Port);

            // Add TXT records used by the Dart implementation
            profile.AddProperty("ble-service-uuids", "0xFC82,0x1818,0x1826,00000001-19CA-4651-86E5-FA29DCDD09D1,A026EE0D-0A7D-4AB3-97FA-F1500F9FEB8B");
            profile.AddProperty("mac-address", "68-67-25-6C-66-9C");
            profile.AddProperty("serial-number", "234700181");

            // Probe then advertise
            if (!sd.Probe(profile))
            {
        ...
        {
            // we optionally log queries. The ServiceDiscovery advertising above will handle answers,
            // but keeping this hook lets you respond to specific questions if desired.
            // Example: if you want to craft manual message answers similar to Dart's MDNSServer usage,
            // build a Message and call mdns.SendAnswer(message);
            var remote = e.RemoteEndPoint;
            System.Diagnostics.Debug.WriteLine($"[{DateTime.Now:O}] mDNS Query from {remote.Address}:{remote.Port}");

            if (e?.Message == null) return;

            foreach (var q in e.Message.Questions)
            {
                System.Diagnostics.Debug.WriteLine($"Question: Name={q.Name}, Type={q.Type}, Class={q.Class}");
            }

            // show answers/authorities/additionals with useful fields
            foreach (var rec in e.Message.Answers)
            {
                System.Diagnostics.Debug.WriteLine($"Answer ({rec.GetType().Name}): {rec}");
            }

            foreach (var srv in e.Message.Answers.OfType<SRVRecord>())
            {
                System.Diagnostics.Debug.WriteLine($"  SRV => Target={srv.Target}, Port={srv.Port}, Name={srv.Name}, TTL={srv.TTL}");
            }
            foreach (var txt in e.Message.Answers.OfType<TXTRecord>())
            {
                System.Diagnostics.Debug.WriteLine($"  TXT => Name={txt.Name}, Entries={txt.Strings.Count}, TTL={txt.TTL}");
                foreach (var s in txt.Strings) System.Diagnostics.Debug.WriteLine($"    {s}");
            }
            foreach (var a in e.Message.Answers.OfType<ARecord>())
            {
                System.Diagnostics.Debug.WriteLine($"  A => {a.Name} -> {a.Address}");
            }
        }
